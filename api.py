...
API_TEMPLATE = 'https://pan.baidu.com/rest/2.0/xpan/{0}'
...
class PCS(BaseClass):
    ...
    """
    def uinfo(self, **kwargs):
        """获取用户信息.

        :return: Response 对象
        """

        return self._request('nas', 'uinfo', **kwargs)
    ...
    def file_list(self, remote_path, order=None, desc=None,
                   limit=None, **kwargs):
        """获取目录下的文件列表.

        :param remote_path: 网盘中目录的路径，默认 / 开头。
                            ...
                            .. warning::
                                ...
                                * 文件名或路径名开头结尾不能是 ``.``
                                  或空白字符，空白字符包括：
                                  ``\\r, \\n, \\t, 空格, \\0, \\x0B`` 。
        :param order: 排序字段，缺省根据文件类型排序：

                   * time（修改时间）
                   * name（文件名）
                   * size（大小，注意目录无大小）
        :param desc: “asc”或“desc”，缺省采用降序排序。
